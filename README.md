...
>0.protocol lets agents sign plugins, rotate credentials safely, and build public, verifiable history over time. It separates identity from credentials so agents can be trusted across systems, restarts, and key rotation.

*API keys are not identity. Credentials should be replaceable. **Identity should not be**.*
...
Three verbs: `express`, `own`, `transfer`. **0.protocol provides the substrate on which identity persists.**
...
*0.protocol does not execute actions. It makes agent intent legible to whatever execution layer interprets them. The substrate is intentionally minimal. It exists to be built upon.*
